...
const ExperienceForm = () => {
  const { resumeData, updateExperience, updateSectionTitle } = useResume();
  const { experience, sectionTitles } = resumeData;
  const [editingIndex, setEditingIndex] = useState(null);
  const [editEntry, setEditEntry] = useState(null);
  const [isEditingTitle, setIsEditingTitle] = useState(false);
  const [tempTitle, setTempTitle] = useState(sectionTitles.experience);

  const handleTitleSave = () => {
    updateSectionTitle("experience", tempTitle);
  ...
  const handleEdit = (index) => {
    ...
    setEditEntry({ ...experience[index] });
  };

  const handleDone = () => {
    const newExp = [...experience];
    if (editingIndex === experience.length) {
      newExp.push(editEntry);
    } else {
      newExp[editingIndex] = editEntry;
    }
  ...
  const toggleVisibility = (index) => {
    const newExp = [...experience];
    newExp[index].visible = !newExp[index].visible;
    updateExperience(newExp);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditEntry({ ...editEntry, company: e.target.value })
                }
          ...
          {isEditingTitle ? (
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  setIsEditingTitle(false);
                  setTempTitle(sectionTitles.experience);
                }}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-xs text-zinc-500 font-medium">
                  {exp.company || "Company"} •{" "}
                  {exp.startDate ? exp.startDate : "Start"} -{" "}
