...
import { billingTopUps, pricingPlans } from "../../constants/pricing";

const catalog = [...pricingPlans, ...billingTopUps];
...
const resolvePricingByPlanId = (planId) => {
  ...
  for (const plan of catalog) {
    ...
    if (plan.monthlyPlanId === planId) {
      return {
        planName: plan.name,
        amount: toPaise(plan.monthlyPrice),
      };
    }

    if (plan.yearlyPlanId === planId) {
      return {
        planName: plan.name,
        amount: toPaise(plan.yearlyPrice),
      };
    }
  }
...
const PaymentStatusContent = ({
  ...
  const transactionData = useMemo(() => {
    const pricingFallback = resolvePricingByPlanId(planId);
    const resolvedAmount =
      Number(
        resultData?.amount ??
          resultData?.amountPaise ??
          resultData?.orderAmount ??
          pricingFallback?.amount ??
          0,
      ) || 0;
    ...
    const base = {
      refId:
        ...
        resultData?.razorpay_payment_id ||
        "Processing...",
      transId:
        resultData?.razorpayOrderId ||
        resultData?.razorpay_order_id ||
        "Processing...",
      // Add explicitly for InvoiceGenerator
      razorpayPaymentId:
        resultData?.razorpayPaymentId || resultData?.razorpay_payment_id,
      razorpayOrderId:
        resultData?.razorpayOrderId || resultData?.razorpay_order_id,
      invoiceId:
  ...
  const isTopup =
    planId?.startsWith("topup_") ||
    ["quick_boost", "power_pack", "pro_master", "student_test"].includes(
      planId,
    );
  ...
  if (verifying) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center space-y-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-white mb-2 uppercase tracking-[0.2em]">
            Verifying
          </h2>
          <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest">
            Confirming with secure gateway...
          </p>
        </div>
      </div>
    );
    ...
    <div className="w-full max-w-lg mx-auto flex flex-col items-center animate-in fade-in zoom-in-95 duration-700">
      ...
      <div className="mb-10 text-center">
        <div className="inline-block relative mb-6">
          <div
            className={`w-20 h-20 rounded-full flex items-center justify-center relative z-10 shadow-2xl transition-all duration-700
          ...
          <div
            className={`absolute -inset-2 rounded-full blur-xl opacity-20 ${isSuccess ? "bg-[#bef264]" : "bg-red-500"}`}
          ></div>
        </div>

        <h1 className="text-2xl font-black text-white mb-2 uppercase tracking-[0.1em]">
      ...
      <div className="w-full bg-[#121214] border border-white/5 rounded-[1.5rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-500 relative">
        {/* Visual Accent */}
        <div
          className={`absolute top-0 left-0 right-0 h-1 ${isSuccess ? "bg-gradient-to-r from-[#bef264] to-transparent" : "bg-red-500"}`}
        ></div>

        <div className="p-5 space-y-6">
          {/* Main Amount & Plan */}
          <div className="text-center">
            <div className="text-[9px] text-zinc-500 font-black uppercase tracking-[0.3em] mb-2">
              Total Amount Paid
            </div>
            ...
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-white/5 rounded-full border border-white/10">
              <span
                className={`w-1.5 h-1.5 rounded-full ${isSuccess ? "bg-[#bef264]" : "bg-red-500"}`}
              ></span>
          ...
          <div className="grid grid-cols-2 gap-y-4 pt-3 border-t border-white/5">
            <div className="space-y-0.5">
              <div className="text-[8px] text-zinc-500 font-black uppercase tracking-[0.2em]">
                Plan
              </div>
            ...
            <div className="space-y-0.5 text-right">
              ...
              <div className="text-[14px] text-white font-bold tracking-tight">
