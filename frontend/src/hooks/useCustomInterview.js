...
export const useCustomInterview = () => {
  ...
  // ✅ FIX: Persistent ref for committed final STT text (survives recognition restarts)
  ...
  // ✅ FIXED startSTT — resolves word repetition on mobile/small screens/all browsers
  const startSTT = useCallback(() => {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) {
      toast.error("Speech recognition not supported.");
    ...
    recognition.onstart = () => {
      // ✅ Reset committed text on each fresh recognition session
    ...
    recognition.onresult = (e) => {
      ...
      // ✅ FIX: Only process NEW results starting from e.resultIndex
      // (old code started from i=0 which caused words to compound on every event)
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (e.results[i].isFinal) {
          committedFinalTextRef.current += e.results[i][0].transcript;
        }
      }

      // ✅ FIX: Only grab the latest interim chunk, not all interim results
      let latestInterim = "";
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (!e.results[i].isFinal) {
      ...
      const displayText = (
        committedFinalTextRef.current + latestInterim
      ).trim();
      ...
      if (displayText) {
        ...
        if (inactivityTimerRef.current)
          clearTimeout(inactivityTimerRef.current);
        isWaitingForInactivityResponseRef.current = false;
        updateUserTranscript(displayText);

        if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);

        if (committedFinalTextRef.current.trim()) {
          // ✅ Primary path: browser confirmed final text — trigger after silence
          silenceTimerRef.current = setTimeout(() => {
            const finalStr = committedFinalTextRef.current.trim();
            if (finalStr) {
              committedFinalTextRef.current = ""; // Clear after sending
              handleUserSpeech(finalStr);
            }
            setIsUserSpeaking(false);
          }, SILENCE_THRESHOLD);
        } else if (latestInterim.trim()) {
          // ✅ Fallback path: some Android browsers never fire isFinal=true
          // Wait longer, then send interim text if still no final arrived
          silenceTimerRef.current = setTimeout(() => {
            const fallback = latestInterim.trim();
            if (fallback && !committedFinalTextRef.current.trim()) {
              committedFinalTextRef.current = "";
              handleUserSpeech(fallback);
            }
            setIsUserSpeaking(false);
          }, SILENCE_THRESHOLD + 1500); // 3500ms total for interim fallback
        }
      }
    };

    recognition.onerror = (e) => {
      // ✅ Reset committed text on error to prevent stale carry-over
      committedFinalTextRef.current = "";
      if (e.error === "not-allowed") {
        setConnectionStatus("Mic Blocked");
        toast.error("Microphone access denied.");
    ...
    recognition.onend = () => {
      // ✅ Reset committed text on end (prevents carry-over into next session)
      committedFinalTextRef.current = "";
      if (
        !hasCallEndedRef.current &&
        !activeCodingTaskRef.current &&
